...
#Purpose: Defines Blinky's AI behavior
...
#point where blinky moves to the other side of the screen
const FLIP_POINT = 592

#target object that blinky moves towards
const BLINKY_TARGET = preload("res://Scenes/blinky_target.tscn")

#handles blinky's animated sprite
@onready var animated_sprite_2d: AnimatedSprite2D = $AnimatedSprite2D

#the position that blinky first starts at
var start_position

#the actual speed that blinky runs at
var speed
#helps check if blinky can go right
var right_way = false
#helps check if blinky can go left
var left_way = false
#helps check if blinky can go down
var down_way = false
#helps check if blinky can go up
var up_way = false

#the current direction blinky is heading towards
var curr_dir = "right"

#helps check if blinky can move
var can_move = true
...
#the amount of dots that determines if blinky turns angry or not 
var angry_amount

#helps determine if blinky is angry
var is_angry = false

#helps determine what state blinky is in
var state = "chase"

#runs as soon as blinky is ready
func _ready():
	...
	corner = get_parent().get_node("BlinkyCorner")
...
func _physics_process(delta):
	#makes sure that blinky goes right when they are at the starting position
	...
	if (target_pos != null):
		...
		#also check if the state equals run
			...
			#set blinky's state to chase
			set_state("chase")
		
		#check if blinky's position is beyond flip point
		if (position.x > FLIP_POINT):
			#move blinky to the other side of the screen
			position.x = -FLIP_POINT
		#check if blinky's position is beyond negative flip point
		elif (position.x < -FLIP_POINT):
			#move blinky to the other side of the screen
			position.x = FLIP_POINT

#signal that runs when blinky collides with a turn point
func _on_turn_point_check_body_entered(body):
	#check if blinky can go right from the turn point
	right_way = body.right_way
	#check if blinky can go left from the turn point
	left_way = body.left_way
	#check if blinky can go up from the turn point
	up_way = body.up_way
	#check if blinky can go down from the turn point
	down_way = body.down_way
	
	#blinky can no longer move
	can_move = false
	#adjust blinky's position to the turn point
	adjust_position(body.position)
	#calculate heuristic values
	calc_heuristics()

#signal that runs when blinky leaves a turn point
func _on_turn_point_check_body_exited(body):
	#can not be determined if blinky can move in any direction
...
#set blinky's position to given parameter
func adjust_position(new_pos):
	#set blinky's position to the new position
	position = new_pos
...
func calc_heuristics():
	...
	#temporary position
	var temp_pos
	#check if blinky can go right
	#also check if blinky is not going left
	if (right_way and curr_dir != "left"):
		#set temp position to blinky's current position
		temp_pos = position
	...
	#check if blinky can go left
	#also check if blinky is not going right
	...
	#check if blinky can go up
	#also check if blinky is not going down
	if (up_way and curr_dir != "down"):
		#set the temp position to blinky's current position
		temp_pos = position
	...
	#check if blinky can go down
	#also check if blinky's current direction is not up
	...
	#check if blinky's state does not equal run
	if (state != "run"):
		#check what direction blinky is moving in
...
func create_target(target_name):
	...
	var my_target = BLINKY_TARGET.instantiate()
...
func set_state(state_name):
	...
	if (state_name == "chase"):
		...
		#set blinky's speed to the normal speed
		speed = SPEED
	elif (state_name == "run"):
		...
		#blinky is no longer angry
		is_angry = false
		#set blinky's speed to the run speed
		speed = RUN_SPEED
	#set the state depending on state name
	state = state_name

#function that handles when super pac-man catches blinky
func kill():
	#set blinky's position to the start position
	position = start_position
	#set blinky's current direction to right
	var curr_dir = "right"
	#blinky can no longer go any direction at the moment
	var right_way = false
	var left_way = false
	var down_way = false
	var up_way = false
